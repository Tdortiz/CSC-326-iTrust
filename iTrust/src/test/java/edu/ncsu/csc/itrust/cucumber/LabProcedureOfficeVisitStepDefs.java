...
public class LabProcedureOfficeVisitStepDefs {
	...
	@Given("^MID: (.*) is greater than 12 years old$")
	public void check_age_of_patient(String mid){
		fail();
	}
	...
	@Given("^Immunization CPT (.*) is in the system$")
	...
	@Given("^Yesterday's office visit has the following Basic Health Metrics: height: (.+) in, weight: (.+) lbs, blood pressure: (.*), LDL: (.+), HDL: (.+), Triglycerides: (.+), Household Smoking Status: (.*), Patient Smoking Status: (.*)$")
	public void check_yesterdays_visit( int height, int weight, String bloodPressure, int ldl, int hdl, int trigs, String hSmoke, String pSmoke ){
		fail();
	}
	...
	public void log_event(String procNaem, String procCode, int labMid){
		fail();
	}
